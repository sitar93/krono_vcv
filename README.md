...
|----------|-------------------|
| **`Krono-2.1.1-win-x64.vcvplugin`** (pattern) | Written by the SDK `plugin.mk` into **`dist/`** — `Krono-` + `plugin.json` version + `-` + arch (`win-x64`, `lin-x64`, `mac-x64`, `mac-arm64`, …). No separate `.zip` from stock `make dist`; the bundle **is** the `.vcvplugin`. |
